...
static
uint16_t LfnBuf[_MAX_LFN + 1];
#define	NAMEBUF(sp,lp)	uint8_t sp[12]; uint16_t *lp = LfnBuf
#define INITBUF(dj,sp,lp)	dj.fn = sp; dj.lfn = lp

#elif _USE_LFN > 1	/* LFN with dynamic LFN working buffer */
#define	NAMEBUF(sp,lp)	uint8_t sp[12]; uint16_t lbuf[_MAX_LFN + 1], *lp = lbuf
#define INITBUF(dj,sp,lp)	dj.fn = sp; dj.lfn = lp

#else				/* No LFN */
#define	NAMEBUF(sp,lp)	uint8_t sp[12]
...
)
{
	...
	if (res == FR_OK) {
		...
		if (fs->fs_type == FS_FAT32 && fs->fsi_flag) {
			...
			ST_uint16_t(fs->win+BS_55AA, 0xAA55);
			ST_uint32_t(fs->win+FSI_LeadSig, 0x41615252);
			ST_uint32_t(fs->win+FSI_StrucSig, 0x61417272);
			ST_uint32_t(fs->win+FSI_Free_Count, fs->free_clust);
			ST_uint32_t(fs->win+FSI_Nxt_Free, fs->last_clust);
...
static
uint32_t get_fat (	/* 0xFFFFFFFF:Disk error, 1:Interal error, Else:Cluster status */
	FATFS *fs,	/* File system object */
...
)
{
	...
	case FS_FAT12 :
		...
		if (move_window(fs, fsect + (bc / SS(fs)))) break;
		wc |= (uint16_t)fs->win[bc & (SS(fs) - 1)] << 8;
		return (clst & 1) ? (wc >> 4) : (wc & 0xFFF);

	case FS_FAT16 :
		if (move_window(fs, fsect + (clst / (SS(fs) / 2)))) break;
		return LD_uint16_t(&fs->win[((uint16_t)clst * 2) & (SS(fs) - 1)]);

	case FS_FAT32 :
		if (move_window(fs, fsect + (clst / (SS(fs) / 4)))) break;
		return LD_uint32_t(&fs->win[((uint16_t)clst * 4) & (SS(fs) - 1)]) & 0x0FFFFFFF;
	}
...
FRESULT put_fat (
	FATFS *fs,	/* File system object */
	uint32_t clst,	/* Cluster# to be changed in range of 2 to fs->max_clust - 1 */
...
)
{
	...
	if (clst < 2 || clst >= fs->max_clust) {	/* Range check */
		...
		case FS_FAT16 :
			...
			if (res != FR_OK) break;
			ST_uint16_t(&fs->win[((uint16_t)clst * 2) & (SS(fs) - 1)], (uint16_t)val);
			break;

		case FS_FAT32 :
			res = move_window(fs, fsect + (clst / (SS(fs) / 4)));
			if (res != FR_OK) break;
			ST_uint32_t(&fs->win[((uint16_t)clst * 4) & (SS(fs) - 1)], val);
			break;
...
)
{
	...
	s = 0;
	do {
		wc = ff_wtoupper(LD_uint16_t(dir+LfnOfs[s]));	/* Get an LFN character */
...
)
{
	...
	do {
		...
		wc = LD_uint16_t(dir+LfnOfs[s]);		/* Get an LFN char */
...
)
{
	...
	dir[LDIR_Type] = 0;
	ST_uint16_t(dir+LDIR_FstClusLO, 0);

	i = (ord - 1) * 13;				/* Get offset in the LFN buffer */
	s = wc = 0;
	do {
		if (wc != 0xFFFF) wc = lfnbuf[i++];	/* Get an effective char */
		ST_uint16_t(dir+LfnOfs[s], wc);	/* Put it */
...
)
{
	...
	res = dir_seek(dj, (uint16_t)((dj->lfn_idx == 0xFFFF) ? i : dj->lfn_idx));	/* Goto the SFN or top of the LFN entries */
	if (res == FR_OK) {
		do {
			res = move_window(dj->fs, dj->sect);
			if (res != FR_OK) break;
...
)
{
	...
	if ((uint32_t)*path < ' ') {			/* Null path means the start directory itself */
		...
		for (;;) {
			...
			dj->sclust = ((uint32_t)LD_uint16_t(dir+DIR_FstClusHI) << 16) | LD_uint16_t(dir+DIR_FstClusLO);
		}
	}
...
)
{
	...
	if (disk_read(fs->drive, fs->win, sect, 1) != RES_OK)	/* Load boot record */
		return 3;
	if (LD_uint16_t(&fs->win[BS_55AA]) != 0xAA55)		/* Check record signature (always placed at offset 510 even if the sector size is >512) */
		return 2;
...
)
{
	...
	*rfs = fs = FatFs[vol];			/* Returen pointer to the corresponding file system object */
	...
	fs->drive = (uint8_t)LD2PD(vol);		/* Bind the logical drive and a physical drive */
	...
	if (fmt == 1) {						/* Not an FAT boot record, it may be patitioned */
		...
		if (tbl[4]) {									/* Is the partition existing? */
			bsect = LD_uint32_t(&tbl[8]);					/* Partition offset in LBA */
			fmt = check_fs(fs, bsect);					/* Check the partition */
		}
	}
	if (fmt == 3) return FR_DISK_ERR;
	if (fmt || LD_uint16_t(fs->win+BPB_BytsPerSec) != SS(fs))	/* No valid FAT patition is found */
		return FR_NO_FILESYSTEM;

	/* Initialize the file system object */
	fsize = LD_uint16_t(fs->win+BPB_FATSz16);				/* Number of sectors per FAT */
	if (!fsize) fsize = LD_uint32_t(fs->win+BPB_FATSz32);
	fs->sects_fat = fsize;
	fs->n_fats = fs->win[BPB_NumFATs];					/* Number of FAT copies */
	fsize *= fs->n_fats;								/* (Number of sectors in FAT area) */
	fs->fatbase = bsect + LD_uint16_t(fs->win+BPB_RsvdSecCnt); /* FAT start sector (lba) */
	fs->csize = fs->win[BPB_SecPerClus];				/* Number of sectors per cluster */
	fs->n_rootdir = LD_uint16_t(fs->win+BPB_RootEntCnt);	/* Nmuber of root directory entries */
	tsect = LD_uint16_t(fs->win+BPB_TotSec16);				/* Number of sectors on the file system */
	if (!tsect) tsect = LD_uint32_t(fs->win+BPB_TotSec32);
	fs->max_clust = mclst = (tsect						/* Last cluster# + 1 */
		- LD_uint16_t(fs->win+BPB_RsvdSecCnt) - fsize - fs->n_rootdir / (SS(fs)/32)
		) / fs->csize + 2;
	...
	if (fmt == FS_FAT32)
		fs->dirbase = LD_uint32_t(fs->win+BPB_RootClus);	/* Root directory start cluster */
	else
	...
	if (fmt == FS_FAT32) {
	 	fs->fsi_flag = 0;
		fs->fsi_sector = bsect + LD_uint16_t(fs->win+BPB_FSInfo);
		if (disk_read(fs->drive, fs->win, fs->fsi_sector, 1) == RES_OK &&
			LD_uint16_t(fs->win+BS_55AA) == 0xAA55 &&
			LD_uint32_t(fs->win+FSI_LeadSig) == 0x41615252 &&
			LD_uint32_t(fs->win+FSI_StrucSig) == 0x61417272) {
			fs->last_clust = LD_uint32_t(fs->win+FSI_Nxt_Free);
			fs->free_clust = LD_uint32_t(fs->win+FSI_Free_Count);
		}
	}
#endif
...
)
{
	...
	res = auto_mount(&path, &dj.fs, (uint8_t)(mode & (FA_WRITE | FA_CREATE_ALWAYS | FA_OPEN_ALWAYS | FA_CREATE_NEW)));
#else
	mode &= FA_READ;
	...
	if (mode & (FA_CREATE_ALWAYS | FA_OPEN_ALWAYS | FA_CREATE_NEW)) {
		...
		if (res != FR_OK) {			/* No file, create new */
			...
			if (mode & FA_CREATE_ALWAYS) {		/* Resize it to zero on over write mode */
				cl = ((uint32_t)LD_uint16_t(dir+DIR_FstClusHI) << 16) | LD_uint16_t(dir+DIR_FstClusLO);	/* Get start cluster */
				ST_uint16_t(dir+DIR_FstClusHI, 0);	/* cluster = 0 */
				ST_uint16_t(dir+DIR_FstClusLO, 0);
				ST_uint32_t(dir+DIR_FileSize, 0);	/* size = 0 */
				dj.fs->wflag = 1;
		...
		if (mode & FA_CREATE_ALWAYS) {
			...
			ps = get_fattime();
			ST_uint32_t(dir+DIR_CrtTime, ps);		/* Created time */
			dj.fs->wflag = 1;
	...
	fp->org_clust =						/* File start cluster */
		((uint32_t)LD_uint16_t(dir+DIR_FstClusHI) << 16) | LD_uint16_t(dir+DIR_FstClusLO);
	fp->fsize = LD_uint32_t(dir+DIR_FileSize);	/* File size */
	...
	for ( ;  btw;									/* Repeat until all data transferred */
		...
		if ((fp->fptr % SS(fp->fs)) == 0) {			/* On the sector boundary? */
			...
			if (cc) {								/* Write maximum contiguous sectors directly */
				if (fp->csect + cc > fp->fs->csize)	/* Clip at cluster boundary */
					cc = fp->fs->csize - fp->csect;
				if (disk_write(fp->fs->drive, wbuff, sect, (uint8_t)cc) != RES_OK)
					ABORT(fp->fs, FR_DISK_ERR);
#if _FS_TINY
	...
	if (res == FR_OK) {
		if (fp->flag & FA__WRITTEN) {	/* Has the file been written? */
			...
			if (res == FR_OK) {
				...
				ST_uint32_t(dir+DIR_FileSize, fp->fsize);		/* Update file size */
				ST_uint16_t(dir+DIR_FstClusLO, fp->org_clust);	/* Update start cluster */
				ST_uint16_t(dir+DIR_FstClusHI, fp->org_clust >> 16);
				tim = get_fattime();			/* Updated time */
				ST_uint32_t(dir+DIR_WrtTime, tim);
				fp->flag &= ~FA__WRITTEN;
				fp->fs->wflag = 1;
				res = sync(fp->fs);
			}
		...
		if (res == FR_OK) {					/* Follow completed */
			...
			if (!dir) {
				...
				if (dir[DIR_Attr] & AM_DIR)	/* Reached to the dir */
					dj.fs->cdir = ((uint32_t)LD_uint16_t(dir+DIR_FstClusHI) << 16) | LD_uint16_t(dir+DIR_FstClusLO);
				else
...
)
{
	...
	if (fat == FS_FAT12) {
		...
		do {
			...
			if (fat == FS_FAT16) {
				if (LD_uint16_t(p) == 0) n++;
				p += 2; i -= 2;
			} else {
				if (LD_uint32_t(p) == 0) n++;
				p += 4; i -= 4;
			}
		} while (--clst);
	}
...
)
{
	...
	if (dir[DIR_Attr] & AM_RDO)				/* Is it a R/O object? */
		LEAVE_FF(dj.fs, FR_DENIED);
	dclst = ((uint32_t)LD_uint16_t(dir+DIR_FstClusHI) << 16) | LD_uint16_t(dir+DIR_FstClusLO);
...
)
{
	...
	tim = get_fattime();
	ST_uint32_t(dir+DIR_WrtTime, tim);
	ST_uint16_t(dir+DIR_FstClusLO, dclst);
	ST_uint16_t(dir+DIR_FstClusHI, dclst >> 16);
	mem_cpy(dir+32, dir, 32); 			/* Create ".." entry */
	dir[33] = '.';
	pclst = dj.sclust;
	if (dj.fs->fs_type == FS_FAT32 && pclst == dj.fs->dirbase)
		pclst = 0;
	ST_uint16_t(dir+32+DIR_FstClusLO, pclst);
	ST_uint16_t(dir+32+DIR_FstClusHI, pclst >> 16);
	...
	if (res != FR_OK) {
		...
		ST_uint32_t(dir+DIR_WrtTime, tim);			/* Crated time */
		ST_uint16_t(dir+DIR_FstClusLO, dclst);		/* Table start cluster */
		ST_uint16_t(dir+DIR_FstClusHI, dclst >> 16);
		dj.fs->wflag = 1;
		res = sync(dj.fs);
	}
	...
	if (res == FR_OK) {
		...
		if (res == FR_OK) {
			...
			if (!dir) {				/* Root directory */
				...
				ST_uint16_t(dir+DIR_WrtTime, fno->ftime);
				ST_uint16_t(dir+DIR_WrtDate, fno->fdate);
				dj.fs->wflag = 1;
				res = sync(dj.fs);
			}
		}
	...
	if (res == FR_NO_FILE) { 					/* Is it a valid path and no name collision? */
		...
		if (res == FR_OK) {
			...
			if (dir[DIR_Attr] & AM_DIR) {		/* Update .. entry in the directory if needed */
				dw = clust2sect(dj_new.fs, (uint32_t)LD_uint16_t(dir+DIR_FstClusHI) | LD_uint16_t(dir+DIR_FstClusLO));
				if (!dw) {
					res = FR_INT_ERR;
				} else {
					...
					if (res == FR_OK && dir[1] == '.') {
						...
						ST_uint16_t(dir+DIR_FstClusLO, dw);
						ST_uint16_t(dir+DIR_FstClusHI, dw >> 16);
						dj_new.fs->wflag = 1;
					}
				}
			}
...
FRESULT f_mkfs (
	...
	uint16_t allocsize		/* Allocation unit size [bytes] */
)
{
	static const uint32_t sstbl[] = { 2048000, 1024000, 512000, 256000, 128000, 64000, 32000, 16000, 8000, 4000,   0 };
	static const uint16_t cstbl[] =  {   32768,   16384,   8192,   4096,   2048, 16384,  8192,  4096, 2048, 1024, 512 };
	uint8_t fmt, m, *tbl;
	...
	if (!partition) {
		...
		ST_uint32_t(tbl, 0x00010180);		/* Partition start in CHS */
		if (n_disk < 63UL * 255 * 1024) {	/* Partition end in CHS */
			n_disk = n_disk / 63 / 255;
			tbl[7] = (uint8_t)n_disk;
			tbl[6] = (uint8_t)((n_disk >> 2) | 63);
		} else {
			ST_uint16_t(&tbl[6], 0xFFFF);
		}
		tbl[5] = 254;
		if (fmt != FS_FAT32)			/* System ID */
			...
			tbl[4] = 0x0c;
		ST_uint32_t(tbl+8, 63);			/* Partition start in LBA */
		ST_uint32_t(tbl+12, n_part);		/* Partition size in LBA */
		ST_uint16_t(tbl+64, 0xAA55);		/* Signature */
	...
	mem_set(tbl, 0, SS(fs));
	ST_uint32_t(tbl+BS_jmpBoot, 0x90FEEB);			/* Boot code (jmp $, nop) */
	ST_uint16_t(tbl+BPB_BytsPerSec, SS(fs));		/* Sector size */
	tbl[BPB_SecPerClus] = (uint8_t)allocsize;		/* Sectors per cluster */
	ST_uint16_t(tbl+BPB_RsvdSecCnt, n_rsv);			/* Reserved sectors */
	tbl[BPB_NumFATs] = N_FATS;					/* Number of FATs */
	ST_uint16_t(tbl+BPB_RootEntCnt, SS(fs) / 32 * n_dir); /* Number of rootdir entries */
	if (n_part < 0x10000) {						/* Number of total sectors */
		ST_uint16_t(tbl+BPB_TotSec16, n_part);
	} else {
		ST_uint32_t(tbl+BPB_TotSec32, n_part);
	}
	tbl[BPB_Media] = partition;					/* Media descripter */
	ST_uint16_t(tbl+BPB_SecPerTrk, 63);				/* Number of sectors per track */
	ST_uint16_t(tbl+BPB_NumHeads, 255);				/* Number of heads */
	ST_uint32_t(tbl+BPB_HiddSec, b_part);			/* Hidden sectors */
	n = get_fattime();							/* Use current time as a VSN */
	if (fmt != FS_FAT32) {
		ST_uint32_t(tbl+BS_VolID, n);				/* Volume serial number */
		ST_uint16_t(tbl+BPB_FATSz16, n_fat);		/* Number of secters per FAT */
		tbl[BS_DrvNum] = 0x80;					/* Drive number */
		tbl[BS_BootSig] = 0x29;					/* Extended boot signature */
		mem_cpy(tbl+BS_VolLab, "NO NAME    FAT     ", 19);	/* Volume lavel, FAT signature */
	} else {
		ST_uint32_t(tbl+BS_VolID32, n);			/* Volume serial number */
		ST_uint32_t(tbl+BPB_FATSz32, n_fat);		/* Number of secters per FAT */
		ST_uint32_t(tbl+BPB_RootClus, 2);			/* Root directory cluster (2) */
		ST_uint16_t(tbl+BPB_FSInfo, 1);				/* FSInfo record offset (bs+1) */
		ST_uint16_t(tbl+BPB_BkBootSec, 6);			/* Backup boot record offset (bs+6) */
		tbl[BS_DrvNum32] = 0x80;				/* Drive number */
		tbl[BS_BootSig32] = 0x29;				/* Extended boot signature */
		mem_cpy(tbl+BS_VolLab32, "NO NAME    FAT32   ", 19);	/* Volume lavel, FAT signature */
	}
	ST_uint16_t(tbl+BS_55AA, 0xAA55);				/* Signature */
	if (SS(fs) > 512U) {
		ST_uint16_t(tbl+SS(fs)-2, 0xAA55);
	}
	...
	for (m = 0; m < N_FATS; m++) {
		...
		if (fmt != FS_FAT32) {
			...
			n |= partition;
			ST_uint32_t(tbl, n);				/* Reserve cluster #0-1 (FAT12/16) */
		} else {
			ST_uint32_t(tbl+0, 0xFFFFFFF8);	/* Reserve cluster #0-1 (FAT32) */
			ST_uint32_t(tbl+4, 0xFFFFFFFF);
			ST_uint32_t(tbl+8, 0x0FFFFFFF);	/* Reserve cluster #2 for root dir */
		}
	...
	if (fmt == FS_FAT32) {
		ST_uint16_t(tbl+BS_55AA, 0xAA55);
		ST_uint32_t(tbl+FSI_LeadSig, 0x41615252);
		ST_uint32_t(tbl+FSI_StrucSig, 0x61417272);
		ST_uint32_t(tbl+FSI_Free_Count, n_clst - 1);
		ST_uint32_t(tbl+FSI_Nxt_Free, 0xFFFFFFFF);
